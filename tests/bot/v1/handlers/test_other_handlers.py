...
@pytest.mark.asyncio
async def test_basic_commands(tg_command: str, dp: Dispatcher, mock_bot: MockedBot) -> None:
    assert not mock_bot.session.requests
    user_id = 1234567
    message = create_tg_command_message(tg_command, user_id)

    # пользователь не зареган
    assert not await UserService.is_user_registered(user_tg_id=message.from_user.id)

    result = await dp.feed_update(mock_bot, Update(message=message, update_id=1))
    assert result is not UNHANDLED
    outgoing_message: TelegramType = mock_bot.get_request()
    assert isinstance(outgoing_message, SendMessage)

    expected_text = (
        START_MESSAGE.format(user_full_name=message.from_user.full_name, msg=LEXICON[tg_command])
        if tg_command == "/start"
...
@pytest.mark.asyncio
async def test_start_command_for_registered_user(registered_user, dp: Dispatcher, mock_bot: MockedBot) -> None:
    assert not mock_bot.session.requests
    user_id = 1234567
    message = create_tg_command_message("/start", user_id)
    ...
    assert outgoing_message.text == (
        START_MESSAGE.format(user_full_name=message.from_user.full_name, msg=LEXICON["/start"])
    )
